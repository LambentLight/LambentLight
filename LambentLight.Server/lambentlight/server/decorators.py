...
def requires_build(func):
    ...
    async def decorator(self):
        matches = [x for x in self.request.app["manager"].builds if x.name == self.request.match_info["name"]]
        if not matches:
...
def requires_folder(func):
    ...
    """
    async def decorator(self):
        matches = [x for x in self.request.app["manager"].folders if x.name == self.request.match_info["name"]]
        if not matches:
...
def requires_server(func):
    ...
    """
    async def decorator(self):
        matches = [x for x in self.request.app["manager"].folders if x.name == self.request.match_info["name"] and x.is_running]
        if not matches:
